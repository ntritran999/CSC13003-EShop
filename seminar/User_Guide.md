...
- [1. Introduction](#1-introduction)
  ...
  - [1.2 Solution with Postman and Postman AI](#12-solution-with-postman-and-postman-ai)
  - [1.3 Target Users (Who the Tool Is For)](#13-target-users-who-the-tool-is-for)
...
- [5. Troubleshooting](#5-troubleshooting)
  - [Error 1: Authentication and Authorization Errors (401 Unauthorized)](#error-1-authentication-and-authorization-errors-401-unauthorized)
  - [Error 2: 400 Bad Request Due to Invalid JSON Payload Structure](#error-2-400-bad-request-due-to-invalid-json-payload-structure)
  - [Error 3: Broken Data Flow in the Pipeline](#error-3-broken-data-flow-in-the-pipeline)
  - [Error 4: Test Failures Due to Incorrect AI-Generated Test Scripts](#error-4-test-failures-due-to-incorrect-ai-generated-test-scripts)
  - [Error 5: 404 Not Found Due to Incorrect Endpoint Routing](#error-5-404-not-found-due-to-incorrect-endpoint-routing)
  - [Error 6: Data Conflicts During Repeated Automated Executions](#error-6-data-conflicts-during-repeated-automated-executions)
- [6. Failure Modes](#6-failure-modes)
  - [Mode 1: Non-existent API Hallucination (API Hallucination)](#mode-1-non-existent-api-hallucination-api-hallucination)
  - [Mode 2: Data Field Hallucination (Payload Hallucination)](#mode-2-data-field-hallucination-payload-hallucination)
  - [Mode 3: Missing Mandatory Data False Positive (Missing Validation False Positive)](#mode-3-missing-mandatory-data-false-positive-missing-validation-false-positive)
  - [Mode 4: Missing Business Calculation Deviations (Business Logic Bypass)](#mode-4-missing-business-calculation-deviations-business-logic-bypass)
...
The scenario chosen for this section is the end-to-end flow for creating a new coupon as an admin, then applying it as a user. This section focuses on demonstrating how to run the collection for the scenario.
...
- Run *User Apply Coupon* request using `userId` and `couponCode` saved from previous requests.
...
Postman also supports CI/CD configuration for automated collection runs. You can find the settings to work with CI/CD when choosing the Automate Runs option in the Collection Runner dashboard.
...
For our E2E coupon collection, we decided to use GitHub Actions as the CI/CD provider. The workflow skeleton was copied from the Postman dashboard, then modified to include steps to install dependencies and start the EShop backend. We also changed the generated workflow from running on push, meaning that the collection would run on CI/CD for every commit pushed to the main branch, to running manually (useful for later demonstration). For the API key, we created a secret key in our EShop's GitHub repository and copied the key value to it.
...
## 7. References
- [Postman quickstart](https://learning.postman.com/docs/getting-started/quick-start)
- [Postbot documentation](https://learning.postman.com/v11/docs/getting-started/basics/about-postbot)
