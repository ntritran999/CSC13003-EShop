  ...
  for (const tc of adminCouponTestData) {
    test(`${tc.id} [${tc.type}] - ${tc.expectedOutcome}`, async ({ page }) => {
      ...
      if (tc.expectedOutcome === 'success') {
        ...
        const expectedDiscountText =
        tc.couponType === 'percent'
          ? `${tc.discountValue}%`
          : new RegExp(`${tc.discountValue}`.replace(/\000$/, '').replace(/000$/, '.*000.*₫'));
        await expect(couponPage.rowFor(tc.code)).toContainText(expectedDiscountText);
      } 
      else if (tc.expectedOutcome === 'validation_error') {
        await couponPage.submitButton.click();
        
        const isValid = await couponPage.discountValueInput.evaluate(
          (el: HTMLInputElement) => el.validity.valid
        );
        expect(isValid).toBe(false);
      }
      else if (tc.expectedOutcome === 'error') {
